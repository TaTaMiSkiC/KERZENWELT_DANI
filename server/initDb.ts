...
async function createDefaultAdmin() {
  ...
  const hashedPassword = await hashPassword("Marija2401");
  ...
  await db.insert(users).values([
    ...
    {
      username: "MiskicAdmin",
      password: hashedPassword,
      email: "v.miskic2@gmail.com",
      isAdmin: true,
      firstName: "Admin",
      lastName: "Vinko Miskic",
    },
  ]);
...
async function createDefaultProducts() {
  ...
  const kerzenboxKategorija = categoryList.find(
    (cat) => cat.name === "Kerzenbox",
  );
  const dekorativneKategorija = categoryList.find(
    (cat) => cat.name === "Dekorativne svijeće",
  );

  if (!kerzenboxKategorija || !dekorativneKategorija) {
    console.error("Kategorije nisu pronađene, preskačem kreiranje proizvoda");
    return;
  }
  ...
  await db.insert(products).values([
    {
      ...
      categoryId: kerzenboxKategorija.id,
      stock: 12,
      scent: "Vanilija",
      color: "Krem",
      burnTime: "40-50 sati",
...
async function checkContactSettingsExist() {
  const [setting] = await db
    .select()
    .from(settings)
    .where(eq(settings.key, "contact_address"));
  return !!setting;
}
